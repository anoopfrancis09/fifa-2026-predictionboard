-- Scope matches to leagues.
-- Existing matches are assigned to the requested league so they only appear there.

do $$
declare
  v_default_league_id uuid := 'e0704676-be8c-4ac4-be0e-99c288bb20f7'::uuid;
begin
  if not exists (
    select 1
    from public.leagues
    where id = v_default_league_id
  ) then
    raise exception 'Default match league % does not exist.', v_default_league_id;
  end if;
end;
$$;
...
set league_id = 'e0704676-be8c-4ac4-be0e-99c288bb20f7'::uuid;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'matches_league_id_fkey'
...
create index if not exists matches_league_status_time_idx
on public.matches(league_id, status, match_time);

drop policy if exists matches_select_authenticated on public.matches;
drop policy if exists matches_select_visible_league on public.matches;
create policy matches_select_visible_league
on public.matches
for select
to authenticated
using (public.is_admin() or public.can_view_league(league_id));

create or replace function public.get_admin_match_leagues()
returns table (
  id uuid,
  name text,
  is_private boolean
...
begin
  ...
  if not public.is_admin() then
    raise exception 'Only admins can manage match leagues.';
  end if;
...
begin
  ...
  if p_league_id is null or not exists (select 1 from public.leagues where id = p_league_id) then
    raise exception 'Select a valid league.';
  end if;
  ...
  if p_league_id <> v_match.league_id and exists (
    select 1
    from public.predictions
    where match_id = p_match_id
  ) then
    raise exception 'Cannot change the league after predictions have been placed.';
  end if;

  update public.matches
  set league_id = p_league_id,
      team_a = trim(p_team_a),
...
begin
  ...
  if v_match.league_id <> p_league_id then
    raise exception 'This match is not available in the selected league.';
  end if;
...
begin
  ...
  if not exists (
    ...
    raise exception 'Results are available only after the match is finished in this league.';
  end if;
...
grant execute on function public.get_admin_match_leagues() to authenticated;
grant execute on function public.admin_update_match(uuid, uuid, text, text, numeric, numeric, numeric, timestamptz) to authenticated;
